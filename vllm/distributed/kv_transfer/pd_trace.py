...
Tracing is disabled unless ``VLLM_PD_TRACE_PATH`` is set. Trace failures are
best-effort and must never affect serving correctness.
"""
...
def is_trace_enabled() -> bool:
    """Return whether PD tracing is enabled for this process."""
...
def _resolve_trace_path() -> Path | None:
    ...
    if trace_path.endswith(os.sep) or path.is_dir():
        ...
        path = path / f"{role}-{os.getpid()}.trace.jsonl"
    return path
...
def trace_event(
    ...
    **fields: Any,
    ...
    """Append one event when tracing is enabled."""
