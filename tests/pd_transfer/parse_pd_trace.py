...
REQUEST_EVENTS = [
    ...
    "pull_transfer_end",
    "pull_kv_recv_done",
    "push_prefill_finished",
    "push_registration_start",
    "push_registration_end",
    "push_transfer_start",
    "push_transfer_sleep_start",
    "push_transfer_sleep_end",
    "push_transfer_end",
    "push_kv_recv_done",
...
BENCH_RESULT_RE = re.compile(
    r"(?P<mode>pull|push)-sleep-(?P<sleep_ms>\d+(?:\.\d+)?)-input-"
...
def nested_percentile(data: dict[str, Any], metric: str, pct: int) -> float | str:
    candidates = [
        ...
        if isinstance(value, dict):
            ...
            for item in value:
                if not isinstance(item, (list, tuple)) or len(item) != 2:
                    continue
                item_pct, item_value = item
                if as_float(item_pct) == float(pct):
                    return as_float(item_value)
    return ""


def percentile_metric(data: dict[str, Any], metric: str, pct: int) -> float | str:
    direct = metric_value(data, f"p{pct}_{metric}_ms", f"P{pct}_{metric}_ms",
                          f"{metric}_p{pct}_ms", f"{metric}_P{pct}_ms")
    return direct if direct != "" else nested_percentile(data, metric, pct)
...
def parse_benchmark_results(root: Path) -> list[dict[str, Any]]:
    ...
    rows.sort(key=lambda r: (r["mode"], r["sleep_ms"], r["input_len"],
                             r["output_len"], r["concurrency"]))
    return rows
...
def build_timelines(root: Path) -> list[dict[str, Any]]:
    ...
    for record in iter_trace_records(root):
        ...
        for req_id in req_ids(record):
            ...
            row.setdefault("mode", record.get("mode", ""))
            ...
            if event.endswith("transfer_end"):
                row[f"{event}_injected_sleep_ms"] = record.get(
                    "injected_sleep_ms", "")
    rows = []
    for row in timelines.values():
        mode = row.get("mode") or ("push" if "push_transfer_end_perf_ns" in row
                                   else "pull")
        row["mode"] = mode
        ...
        row["proxy_ttft_ms"] = to_ms(delta("proxy_request_received",
                                           "proxy_first_response_chunk"))
        if mode == "push":
            row["transfer_wall_ms"] = to_ms(delta("push_transfer_start",
                                                  "push_transfer_end"))
            row["prefill_to_transfer_end_ms"] = to_ms(delta(
                "push_prefill_finished", "push_transfer_end"))
            row["decode_start_to_transfer_end_ms"] = to_ms(delta(
                "proxy_decode_request_start", "push_transfer_end"))
        else:
        ...
        rows.append(row)
    rows.sort(key=lambda r: (r.get("mode", ""), r.get("request_id", "")))
    return rows
