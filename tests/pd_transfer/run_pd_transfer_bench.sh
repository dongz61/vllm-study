...
for value_name in NIXL_PACKED_STAGING_MIB NIXL_PACKED_STAGING_SLOTS \
  NIXL_PACKED_AUTO_RANGE_THRESHOLD; do
  value=${!value_name}
...
mkdir -p "${RUN_ROOT}"
printf '{"canonicalize_reverse_block_pairs":%s,"nixl_transfer_mode":"%s","nixl_packed_staging_mib":%s,"nixl_packed_staging_slots":%s,"nixl_packed_auto_range_threshold":%s}\n' \
